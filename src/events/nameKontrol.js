...
\` ➥ \` Değiştirme Tarihi: <t:${Math.floor(Date.now() / 1000)}:R>`);
    ...
    if (newMember.guild.channels.cache.find(c => c.name.includes(isimcek.kanalcek.namelog))) newMember.guild.channels.cache.find(c => c.name.includes(isimcek.kanalcek.namelog)).send({ embeds: [Jaylen] });
