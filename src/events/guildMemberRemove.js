...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.wsend({ content: ` 
> <a:vendora:1221466255991635998> \`${member.user.tag}\` İsimli Üye <t:${Math.floor(Date.now() / 1000)}> Tarihinde Sunucumuzdan Ayrıldı Ama Kimin Davet Ettiği Bulunamadı.`});
  } else {
    ...
    channel.wsend({ content:`
> <a:vendora:1221466255991635998> \`${member.user.tag}\` İsimli Üye <t:${Math.floor(Date.now() / 1000)}> Tarihinde Sunucumuzdan Ayrıldı. **${inviter.tag}** Tarafından Davet Edilmişti. (\`${total} davet\`)`});
