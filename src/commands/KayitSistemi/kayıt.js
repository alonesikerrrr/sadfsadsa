...
module.exports = {
  conf: {
    ...
    name: "kayıt",
    help: "kayıt <@adonis/ID> <Isim> <Yaş>",
    category: "kayıt",
  },
  
run: async (client, message, args, embed, prefix) => { 
            if(!message.channel.name.includes("welcome-to-vendora")) {
            return message.channel.send(`**Komut sadece <#1221439648413253794> kanalında kullanılabilir.**`)
}
...
`)]})
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}kayıt <@Shadow/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
...
${green} Kişisinin toplamda **${data ? `${data.names.length}` : "0"}** isim kayıtı bulundu
...
<@&1221444722560864347>, <@&1221444722053349457> rolleri verilerek **Erkek** olarak kayıt edildi. ${green}
`)
  if(msg) msg.delete();
msg.channel.send({ embeds: [ozie], components: [row2]});
    ...
    if(conf.chatChannel && client.channels.cache.has(conf.chatChannel)) client.channels.cache.get(conf.chatChannel).send({ content:`Aramıza **${uye}** yakışıklısı katıldı onu Merhaba ile karşılayın.`}).then((e) => setTimeout(() => { e.delete(); }, 95000));
...
message.react(green)
 let aspectt = new MessageEmbed()
 .setColor("PURPLE")
.setDescription(`
${uye.toString()} Üyesinin ismi \`"${setName}"\` olarak değiştirildi.

<@&1221444724972322966>, <@&1221444724012089397> rolleri verilerek **Kadın** olarak kayıt edildi. ${green}
`)
  if(msg) msg.delete();
msg.channel.send({ embeds: [aspectt], components: [row2]});
    ...
    if(conf.chatChannel && client.channels.cache.has(conf.chatChannel)) client.channels.cache.get(conf.chatChannel).send({ content:`Aramıza **${uye}** güzelliği katıldı onu Merhaba ile karşılayın.`}).then((e) => setTimeout(() => { e.delete(); }, 85000));
