...
run: async (client, message, args, prefix) => { 
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}isim <@User/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
...
if (banNum == 5) {
    return message.channel.send(`Gün içerisinde çok fazla İsim işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.`);
}
    if(!isim && !yaş) 
    {
    message.react(red)
...
.setFooter({ text: `Üyesinin önceki isimlerine <!isimler @User/ID> komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.` });
