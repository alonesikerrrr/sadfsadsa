  ...
  run: async (client, message, args) => {
    ...
    .addComponents(
        new MessageSelectMenu()
            ...
            .addOptions([
                { label: 'Sunucu Giriş',description: 'Sunucuya Giriş Tarihiniz.', value: 'I', emoji: { "name": "✨" },},
                { label: 'Hesap Açılış',description: 'Hesabınızın Açılış Tarihi.', value: 'III', emoji: { "name": "✨" },},
                { label: 'Rolleriniz',description: 'Üzerinizde Bulunan Roller.', value: 'II', emoji: { "name": "✨" },},
                { label: 'Aktiflik',description: 'Sunucudaki Anlık Aktif Listesini Görüntüleyin.', value: 'VI', emoji: { "name": "✨" },},
                { label: 'Davet Stat',description: 'Sunucudaki Davet Bilgileriniz.', value: 'IV', emoji: { "name": "✨" },},
                { label: 'Mesaj Stat',description: 'Sunucudaki Mesaj Bilgileriniz.', value: 'VIII', emoji: { "name": "✨" },},
                { label: 'Ses Stat',description: 'Sunucudaki Ses Bilgileriniz.', value: 'IX', emoji: { "name": "✨" },},
                { label: 'İsim Geçmişi',description: 'Önceki İsim Bilgileriniz.', value: 'VII', emoji: { "name": "✨" },},
             ]),
    );
...
let msg = await message.channel.send({content: `**Vendora ✦** Sunucu İçi Bilgilerinizi, Bot Komutlarını Ve Diğer Herşeyi Buradan Öğrenebilirsiniz.`, components: [bpanelrow,kısayollar] });
...
{
await interaction.reply({ content: `<a:bitt:1217922298271502448> **Sunucuya Giriş Tarihiniz :** <t:${Math.floor(interaction.member.joinedTimestamp / 1000)}> (<t:${Math.floor(interaction.member.joinedTimestamp / 1000)}:R>)`, ephemeral: true });
}

if(interaction.values[0] === "II")
{
await interaction.reply({ content: `<a:bitt:1217922298271502448> **Üzerinde Bulunan Rollerin Listesi ;**
...
{
await interaction.reply({ content: `<a:bitt:1217922298271502448> **Hesabınız** <t:${Math.floor(member.createdTimestamp / 1000)}>  (<t:${Math.floor(member.createdTimestamp / 1000)}:R>) **Tarihinde Açılmış**`, ephemeral: true });
}

if(interaction.values[0] === "IV")
{
await interaction.reply({ content: `
<a:bitt:1217922298271502448> ${member.toString()}, <t:${Math.floor(Date.now() / 1000)}> Tarihine Kadar Sunucumuzda Yapmış Olduğunuz Tüm İnvite İstatislikleriniz Aşağıda Belirtilmiştir.
${miniicon} **Toplam** \`${regular}\` **Davetin Bulunmakta.**
...
await interaction.reply({ content: `${green} ${member.toString()} üyesi başarıyla kayıtsıza atıldı!`, ephemeral: true });
}

if(interaction.values[0] === "VI")
{
await interaction.reply({ content: `
${miniicon} **Sesli Kanallardaki Toplam Üye Sayısı :** \`${(interaction.guild.members.cache.filter((x) => x.voice.channel).size)}\`
...
${miniicon} **Sunucu Destek/Id Numarası :** \`${(interaction.guild.id)}\`
`, ephemeral: true });
}
...
{
await interaction.reply({ content: `
<a:bitt:1217922298271502448> ${member.toString()}, <t:${Math.floor(Date.now() / 1000)}> Tarihine Kadar Sunucumuzda Yapmış Olduğunuz Tüm Mesaj İstatislikleriniz Aşağıda Belirtilmiştir.
...
{
await interaction.reply({ content: `
<a:bitt:1217922298271502448> ${member.toString()}, <t:${Math.floor(Date.now() / 1000)}> Tarihine Kadar Sunucumuzda Yapmış Olduğunuz Tüm Ses İstatislikleriniz Aşağıda Belirtilmiştir.
