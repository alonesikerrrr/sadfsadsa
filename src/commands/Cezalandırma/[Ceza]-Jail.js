...
const { red, green, Revuu, kirmiziok, revusome, star, miniicon } = require("../../configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["jail","temp-jail"],
    name: "jail",
    help: "jail <Shadow/ID> <Süre> <Sebep>",
...
if (duration) {
  ...
  .setColor("#2f3136")
.setDescription(`
<a:bitt:1214235957679886337> ${member.toString()} Adlı Kişiye Jail Atıldı
  
${miniicon} Jail Atan Kişi : ${message.author} (\`${message.author.id}\`)
${miniicon} Ceza Başlangıç: <t:${Math.floor(Date.now() / 1000)}> (<t:${Math.floor(Date.now() / 1000)}:R>)
${miniicon} Ceza Bitiş: <t:${Math.floor((Date.now() + duration) / 1000)}> (<t:${Math.floor((Date.now() + duration) / 1000)}:R>)
...
${miniicon} Jail Atan Kişi : ${message.author} (\`${message.author.id}\`)
${miniicon} Ceza Başlangıç: <t:${Math.floor(Date.now() / 1000)}> (<t:${Math.floor(Date.now() / 1000)}:R>)
${miniicon} Ceza Bitiş: \`Sınırsız\`
${miniicon} Ceza Sebebi: \`Özel / Önemli\`
`)
.setFooter({ text:`${moment(Date.now()).format("LLL")}    (Ceza ID: #${penal.id})` })
