...
const { red, green, Mute, revusome, kirmiziok, star, miniicon } = require("../../configs/emojis.json")

module.exports = {
  conf: {
    aliases: ["mute","cmute","suslan"],
    name: "mute",
    help: "mute <Shadow/ID> <Süre> <Sebep>",
...
if(!message.channel.name.includes("şüpheli-chat")){
    ...
    .addComponents(
        new MessageSelectMenu()
            ...
            .addOptions([
                { label: 'Kışkırtma, Trol ve Dalgacı Davranış (5 Dakika)', value: 'mute1', emoji: { "name": "🛡" }},
                { label: 'Flood,Spam ve Capslock Kullanımı (5 Dakika)', value: 'mute2', emoji: { "name": "🛡" }},
                { label: 'Metin Kanallarını Amacı Dışında Kullanmak (10 Dakika)', value: 'mute3', emoji: { "name": "🛡" }},
                { label: 'Küfür, Argo, Hakaret ve Rahatsız Edici Davranış (5 Dakika)', value: 'mute4', emoji: { "name": "🛡" }},
                { label: 'Abartı, Küfür ve Taciz Kullanımı (30 Dakika)', value: 'mute5', emoji: { "name": "🛡" }},
                { label: 'Dini, Irki ve Siyasi değerlere Hakaret (2 Hafta)', value: 'mute6', emoji: { "name": "🛡" }},
                { label: 'Sunucu Kötüleme ve Kişisel Hakaret (1 Saat)', value: 'mute7', emoji: { "name": "🛡" }},
    ...
    if (duration) {
      ...
      await message.channel.send({ content:`${green} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(green)
      if (conf.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar susturuldunuz.`}).catch(() => {});
    
      const log = embed
        .setAuthor({ name: member.user.username, iconURL:  member.user.displayAvatarURL({ dynamic: true }) })
        .setColor("#2f3136")
.setDescription(`
<a:bitt:1214235957679886337> ${member.toString()} Adlı Kişiye Chat Mutesi Atıldı
  
${miniicon} Mute Atan Kişi : ${message.author} (\`${message.author.id}\`)
${miniicon} Ceza Başlangıç: <t:${Math.floor(Date.now() / 1000)}> (<t:${Math.floor(Date.now() / 1000)}:R>)
${miniicon} Ceza Bitiş: <t:${Math.floor((Date.now() + duration) / 1000)}:R>
${miniicon} Ceza Sebebi: \`${reason}\`
        `)
