...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (conf.Main.dmMessages) member.send({ content: `**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle uyarıldınız!`}).catch(() => {});
    
    const log = embed
    .setAuthor({ name: member.user.username, iconURL:  member.user.displayAvatarURL({ dynamic: true }) })
      .setColor("#2f3136")
      .setDescription(`
<a:bitt:1214235957679886337> ${member.toString()} üyesi uyarıldı!

${miniicon} Ceza ID: \`#${penal.id}\`
${miniicon} Uyarılan Üye: ${member.toString()} \`(${member.user.username.replace(/\`/g, "")} - ${member.user.id})\`
${miniicon} Uyaran Yetkili: ${message.author} \`(${message.author.username.replace(/\`/g, "")} - ${message.author.id})\`
${miniicon} Uyarılma Tarihi: <t:${Math.floor(Date.now() / 1000)}> (<t:${Math.floor(Date.now() / 1000)}:R>)
${miniicon} Uyarı Sayısı: \`${uyariData ? Math.floor(parseInt(uyariData.sayi)) : 1} Uyarı\`
      `)
