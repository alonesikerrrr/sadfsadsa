...
const { red, green, Cezaa, Revuu, kirmiziok, miniicon } = require("../../configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["forceban"],
    name: "forceban",
    help: "forceban <Shadow/ID>",
    category: "sahip",
    owner: true
  },
...
if(!message.channel.name.includes("commands")) {
    ...
    if (conf.Main.dmMessages) user.send({ content :`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle **kalıcı olarak** banlandınız!`}).catch(() => {});
...
${miniicon} Banlanan Üye: **${member ? member.toString() : user.username}** \`${user.id}\`
