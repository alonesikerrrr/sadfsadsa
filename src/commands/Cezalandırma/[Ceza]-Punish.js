...
const { red, green, Mute, revusome, kirmiziok, Cezaa, Revuu, miniicon } = require("../../configs/emojis.json")
...
if (conf.Main.dmMessages) member.send({ content :`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle **kalıcı olarak** banlandınız!`}).catch(() => {});
...
.setDescription(`
<a:bitt:1214235957679886337> Banlayan Yetkili: ${message.author} \`${message.author.id}\`

${miniicon} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
...
if(button.customId === "üç") {
  ...
  return }
  if (conf.Main.banlimit > 0 && banLimit.has(message.author.id) && banLimit.get(message.author.id) == conf.Main.banlimit) return message.channel.send({ content:"Saatlik ban sınırına ulaştın!", ephemeral: true })

  if (conf.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **Reklam** sebebiyle banlandınız!`}).catch(() => {});
