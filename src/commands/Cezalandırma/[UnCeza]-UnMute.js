...
const {red, green, Revuu, star, miniicon} = require("../../configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["unmute","uncmute"],
    name: "unmute",
    help: "unmute <Shadow/ID>",
  ...
  run: async (client, message, args, embed) => {
    ...
    .setLabel("Chat Mute")
    .setStyle("SECONDARY")
    .setEmoji("1217921445921947700");

    let vmute = new MessageButton()
    .setCustomId("vmute")
    .setLabel("Voice Mute")
    .setStyle("SECONDARY")
    .setEmoji("1218277537780858960");
  ...
  collector.on("collect", async (button) => {
    ...
    if (button.customId === "mute") {
      ...
      const log = embed
        ...
        .setColor("#2f3136")
        .setDescription(`
<a:bitt:1217922298271502448> ${member.toString()} Adlı Kişinin Chat Mutesi Kaldırıldı
      
${miniicon} Mute Kaldıran Kişi : ${message.author} (\`${message.author.id}\`)`)
    ...
    if (button.customId === "vmute") {
      ...
      const log = embed
        ...
        .setColor("#2f3136")
.setDescription(`
<a:bitt:1214235957679886337> ${member.toString()} Adlı Kişinin Ses Mutesi Kaldırıldı
      
${miniicon} Mute Cezası Kaldırılan Kişi : ${message.author} (\`${message.author.id}\`)`)
        .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
      message.guild.channels.cache.get(conf.cmuteLogChannel).wsend({ embeds : [log]}); 
