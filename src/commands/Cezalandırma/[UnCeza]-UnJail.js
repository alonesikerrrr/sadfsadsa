...
const { red, green, Revuu, miniicon} = require("../../configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["unjail"],
    name: "unjail",
    help: "unjail <Shadow/ID>",
  ...
  run: async (client, message,  args, embed) => {
    ...
    if (conf.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, jailiniz kaldırıldı!`}).catch(() => {});

    const log = embed
    .setAuthor({ name: member.user.username, iconURL:  member.user.displayAvatarURL({ dynamic: true }) })
      .setColor("#2f3136")
      .setDescription(`
<a:bitt:1214235957679886337> ${member.toString()} Adlı Kişinin Jaili Kaldırıldı
      
${miniicon} Jail Cezası Kaldırılan Kişi : ${message.author} (\`${message.author.id}\`)
${miniicon} Jailin Kaldırımla Tarihi : <t:${Math.floor(Date.now() / 1000)}> (<t:${Math.floor(Date.now() / 1000)}:R>)
          `)
