...
"1221441654809169970",
  ...
  if (!message.member.roles.cache.some(r => conf.botcommandsRolü.includes(r.id)) && !message.member.hasPermission("ADMINISTRATOR")) return;
...
const inviterData = await inviterSchema.findOne({ guildID: message.guild.id, userID: member.user.id });
    ...
    .setCustomId("yükselt")
    .setStyle("SUCCESS")
    .setEmoji("911663299890937856")

    var DüşürButon = new MessageButton()
    .setLabel("Düşür")
    .setCustomId("düşür")
    .setStyle("DANGER")
    .setEmoji("911663313950220368")
 ...
 if(!member.roles.cache.has(ranks[ranks.length-1])){
    ...
    button.reply({ content:`:x: Belirtilen Kullanıcı Zaten Max Role Sahip.`, ephemeral: true})}
      ...
      if(button.customId === "düşür") {
        ...
        await msj.react('<a:Onay:1217922807065870367>');
      const filter = (reaction, user) => {
        return reaction.emoji.name === 'Onay' && user.id === message.author.id;
      };
