...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!args[0]) return message.reply({ content:`Kullanımı: !r al/ver Kullanıcı Rol`})
    if (args[0] != "al") {
        if (args[0] != "ver") {
            return message.reply({ content:`Kullanımı: !r al/ver Kullanıcı Rol`})
        }
    }

    if (!args[1]) return message.reply({ content:`Bir üye etiketle ve tekrardan dene!`})
    let rMember = message.mentions.members.first() || message.guild.members.cache.get(args[1])
    if (!rMember) return message.reply({ content:`Bir üye etiketle ve tekrardan dene!`})

    if (!args[2]) return message.reply({ content:`Rolü belirtmelisin.`})
    let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2])
        ...
        if (message.member.roles.highest.rawPosition <= role.rawPosition) return message.reply({ content:`Kendi rolünden yüksek veya eşit bir rolle işlem yapamazsın.`})
