...
const { green, red, miniicon } = require("../../configs/emojis.json")
...
if(!message.channel.name.includes("yönetici-chat")) {
  ...
  let embed = new MessageEmbed().setAuthor({ name: üye.displayName, iconURL: üye.user.avatarURL({ dynamic: true })}).setTimestamp().setColor(üye.displayHexColor).setFooter({ text: message.author.tag, iconURL: message.author.avatarURL({ dynamic: true })}).setThumbnail(üye.user.avatarURL({ dynamic: true }))
    .addField(`
<:Sivra_isaretle:1130961632915370044> Kullanıcı Bilgisi`,`
${miniicon} Hesap: ${üye}
${miniicon} Kullanıcı ID: ${üye.id}
${miniicon} Kuruluş Tarihi: <t:${Math.floor(member.user.createdTimestamp / 1000)}:R>
`)
    .addField(`
<:Sivra_isaretle:1130961632915370044> Sunucu Bilgisi`,`
${miniicon} Sunucu İsmi: ${nickname}
${miniicon} Katılım Tarihi: <t:${Math.floor(üye.joinedAt / 1000)}:R>
${miniicon} Katılım Sırası: ${(message.guild.members.cache.filter(a => a.joinedTimestamp <= üye.joinedTimestamp).size).toLocaleString()}/${(message.guild.memberCount).toLocaleString()}
...
`);
  if (üye.permissions.has("ADMINISTRATOR") || conf.teyitciRolleri.some(x => üye.roles.cache.has(x))) 
    embed.addField(`
<:Sivra_isaretle:1130961632915370044> Yetkili Bilgisi`,
`
