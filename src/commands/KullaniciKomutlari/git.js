...
.setLabel("Kabul Et")
.setStyle("SUCCESS")
.setEmoji("1217922807065870367"),

new MessageButton()
.setCustomId("red")
.setLabel("Reddet")
.setStyle("DANGER")
.setEmoji("1217922836836909077"),
);
...
.setDisabled(true),
  ...
  if (message.member.permissions.has("ADMINISTRATOR")) {
      ...
      .setColor("#2f3136")
      .setDescription(`
${green} Bir Transport işlemi gerçekleşti.
  
${miniicon} Odaya Giden Kullanıcı: ${member} - \`${member.id}\`
${miniicon} Odasına Gidilen Yetkili: ${message.author} - \`${message.author.id}\`
      `)
      .setFooter({ text: `${moment(Date.now()).format("LLL")}`})
      client.channels.cache.find(x => x.name == "ses-log").wsend({ embeds: [log] });
} else {    
...
.setTimestamp()
.setDescription(`${green} ${message.author}, ${member} kişisinin yanına başarıyla gittiniz.`)
...
.setFooter({ text: message.author.tag, iconURL: message.author.avatarURL({ dynamic: true })})
.setTimestamp()
.setDescription(`${red} ${message.author}, ${member} yanına gitme işlemi iptal edildi.`)
